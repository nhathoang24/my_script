...
(function () {
  ...
  const isTopFrame = pageWindow === pageWindow.top;
  ...
  if (isTopFrame) pageWindow.addEventListener('message', handleResolvedPlaylist);

  function installFetchInterceptor() {
    if (pageWindow.__qmhHlsFetchInstalled) return;
    pageWindow.__qmhHlsFetchInstalled = true;
      ...
      if (requestUrl.includes(RESOLVE_PATH) && response.ok) {
        response.clone().json().then((payload) => {
          ...
          if (payload?.success && typeof playlistUrl === 'string') {
            pageWindow.top.postMessage({ type: MESSAGE_TYPE, playlistUrl }, '*');
          }
        }).catch((error) => console.warn('[QMH HLS] Could not read resolve response:', error));
      }
  ...
  function handleResolvedPlaylist(event) {
    if (!isTopFrame || event.origin !== location.origin) return;
  ...
  async function fetchBuffer(url) {
    const response = await requestWithFetch(url, 'arraybuffer');
    return normalizeArrayBuffer(response, url);
  }
